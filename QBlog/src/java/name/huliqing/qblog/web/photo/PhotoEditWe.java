...
@ManagedBean
@RequestScoped
public class PhotoEditWe extends BaseWe {

    private PhotoEn photo;
    ...
    public PhotoEditWe() {
        super();
        Long tempPhotoId = QFaces.convertToLong(QBlog.getParam("photoId"));
        if (tempPhotoId != null) {
            photo = PhotoSe.findSimple(tempPhotoId);
        }
    }

    public PhotoEn getPhoto() {
        return photo;
    }

    public void setPhoto(PhotoEn photo) {
        this.photo = photo;
    }
    ...
    public String edit() {
        if (PhotoSe.update(photo)) {
            if (returnURL != null) {
                QBlog.redirect(returnURL);
            } else {
                QBlog.redirect("/admin/photo/photoList.faces?folderId=" + photo.getFolder());
            }
        } else {
            Messenger.sendError("编辑失败！");
        }
        return null;
    }
