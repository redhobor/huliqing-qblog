...
import name.huliqing.qblog.Visitor;
...
@ManagedBean
@RequestScoped
public class PhotoDeleteWe extends BaseWe {

    private String result;

    public PhotoDeleteWe() {
        super();
        Long photoId = QFaces.convertToLong(QBlog.getParam("photoId"));
        if (photoId != null) {
            Visitor visitor = QBlog.getCurrentVisitor();
            if (visitor.isLogin()) {
                if (PhotoSe.delete(photoId)) {
                    result = "删除图片成功, photoId=" + photoId;
                    if (returnURL != null) {
