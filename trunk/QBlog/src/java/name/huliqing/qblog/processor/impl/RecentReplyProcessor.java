 ...
 * @author huliqing
 */
public class RecentReplyProcessor extends XmlProcessor2 {

    public List<Attribute2> getRequiredAttributes() {
        List<Attribute2> as = new ArrayList<Attribute2>(5);
        as.add(new AttrInputText("Size", "6", "显示的最近回复数,默认:6"));
        as.add(new AttrSelectBooleanCheckbox("Show Index", "true", "是否显示序号，请填写 true/false,默认:true"));
        as.add(new AttrSelectBooleanCheckbox("Show Date", "false", "是否显示回复日期，请填写true/false, 默认:false"));
        as.add(new AttrInputText("Date Format", "yyyy-MM-dd HH:mm", "回复日期的格式，只有Show Date为true时才有意义,默认:yyyy-MM-dd HH:mm"));
    ...
    public UIComponent render(ModuleEn module) {
        ...
        List<ReplyEn> aes = ReplySe.findLastReplies(size);

        // Create DataTable
        RecentReplyDataTable table = new RecentReplyDataTable();
        table.setValue(aes);
